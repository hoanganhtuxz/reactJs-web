    ...
    useEffect(() => {
        const listtodo = async () => {
            try {
                ...
                let { data: cate } = await CategoryAPI.get(id);
    ...
    return (
        <div id="main-content" className="pt-32">
            <section className="container mx-auto">
                <div className="ml-6">Home &gt; Products &gt; {cate.name}</div>
                    ...
                    <div className="md:col-span-5 ">
                        <div className="mx-auto">
                            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5 mx-auto ">
                                {products.filter(product => product.category == id).map((pr, index) => {
                                    ...
                                    return (
                                        <div key={index} className=" text-center mx-auto hover:text-black hover:no-underline  hover:bg-yellow-100 hover:shadow-2xl border-b-2  hover:border-green-300 pb-6">
                                            <Link to={`/product/${pr._id}`} className>
                                                <img className="transform motion-reduce:transform-none hover:-translate-y-1 hover:scale-110 transition ease-in-out duration-300  mx-auto w-10/12 py-8 h-64 w-64 object-cover " src={`http://localhost:4000/api/productList/image/${pr._id}`} />
